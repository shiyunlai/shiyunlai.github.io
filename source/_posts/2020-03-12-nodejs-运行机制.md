...
在网上找很多资料，也摘抄了很多内容，见[node-运行机制阅读摘抄](./2020-03-31-nodejs-运行机制阅读摘抄.md)看得似懂非懂，大流好像明白，但细节经不起推敲！

这些摘抄内容的解释有点“打开冰箱门，大象放进去，关上冰箱门”的味道，感觉清晰了，但是仔细推敲会有一堆疑问！
于是找了[Node.js官网资料](https://nodejs.org/en/docs/guides/event-loop-timers-and-nexttick/#the-node-js-event-loop-timers-and-process-nexttick)。以下是对Node.js运行机制的整理：
...
Node.js主线程在处理执行栈中的代码时，对任务进行分类到不同的队列，过程中有以下规则（总结出来的）：

从执行node xx.js开始；
V8负责解析xx.js代码并放入执行栈；
执行栈和nextTick Queue中的内容在一次Tick周期内执行，并且主线程清空执行栈后，立即处理nextTick Queue中的任务。
>注意：Call Stack和nextTick Queue不属于EventLoop周期内的队列；
Call Stack和nextTick Queue构成Tick周期；
所有得到主线程处理的代码（current operation）都会放回执行栈（Call Stack）开启一次Tick进行处理；
...
至此，脑袋里有了一个相对清晰的Event Loop大圈，内套多次Tick小圈的Node.js执行流程模型！

整个Node.js运行过程中，哪些I/O、Timer、来自网络的任务统统分类到各种Queue，有Libuv线程池处理，而Node.js主线程只处理同步代码（current operation）。


>上图参考[What is the Event Loop](https://nodejs.org/en/docs/guides/event-loop-timers-and-nexttick/#the-node-js-event-loop-timers-and-process-nexttick)一文整理
